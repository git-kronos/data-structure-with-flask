...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///db.file'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = 0


# configure sqlite3 to enforce foreign key constrains
@event.listens_for(Engine, 'connect')
def _set_sqlite_pragma(db_api_connection, connection_record):
    if isinstance(db_api_connection, SQLite3Connection):
...
def home():
    return jsonify({'message': 'looks good'}), 200
